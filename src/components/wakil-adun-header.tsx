...
export function WakilAdunHeader() {
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <header className={styles.header}>
      <Link href="/wakil-adun" className={styles.logoLink} onClick={() => setMenuOpen(false)}>
      ...
      <nav className={`${styles.navLinks} ${menuOpen ? styles.navLinksActive : ""}`}>
        <Link href="/wakil-adun" onClick={() => setMenuOpen(false)}>
          Papan Utama
        </Link>
        <Link href="/wakil-adun/applications" onClick={() => setMenuOpen(false)}>
          Permohonan
        </Link>
        <Link href="/wakil-adun/report" onClick={() => setMenuOpen(false)}>
          Laporan
        </Link>
